...
class FunctionManager:
  ...
  def _cache_function_parameters_and_return_type(self, tokenized_program):
    ...
    for line_num, line in enumerate(tokenized_program):
      ...
      elif line and line[0] == InterpreterBase.LAMBDA_DEF:
        lambda_name = self.create_lambda_name(line_num)
        lambda_params = [self._to_tuple(formal) for formal in line[1:-1]]
        print(f'line with lambda is : {line}')
        print(f'lambda name is: {lambda_name} and params for it are: {lambda_params}')
        lambda_info = FuncInfo(lambda_params, line_num + 1)
        self.func_cache[lambda_name] = lambda_info
        return_type_stack.append(line[-1])

      if line and line[0] == InterpreterBase.ENDFUNC_DEF:
        reset_after_this_line = True
      elif line and line[0] == InterpreterBase.ENDLAMBDA_DEF:
        reset_after_this_line = True
  ...
  # def create_function_var(self,func_name, ip):
  #   func_info = FuncInfo([], -1) #using ip of -1 for func vars starting line, default no params
  #   self.func_cache[func_name] = func_info
  #   self.return_types[ip] = 'void' #set ip of line where func var is defined to be void as default
